...
export class Lock extends Base {
    ...
    withLockState(property: string, description: string) {
        this.addFeature(
            new Enum("lock_state", access.STATE, ["not_fully_locked", "locked", "unlocked"]).withProperty(property).withDescription(description),
        );
        return this;
    }
...
export class Light extends Base {
    ...
    withColorTemp(range: Range) {
        ...
        const feature = new Numeric("color_temp", access.ALL)
            .withUnit("mired")
    ...
    withColorTempStartup(range: Range) {
        ...
        const feature = new Numeric("color_temp_startup", access.ALL)
            .withUnit("mired")
            .withValueMin(range[0])
            .withValueMax(range[1])
...
export class Climate extends Base {
    ...
    withPiHeatingDemand(access = a.STATE) {
        this.addFeature(
            new Numeric("pi_heating_demand", access)
    ...
    withWeeklySchedule(modes: ThermostatScheduleMode[], access = a.ALL) {
        const featureDayOfWeek = new List(
            "dayofweek",
            a.SET,
            new Composite("day", "dayofweek", a.SET).withFeature(
                new Enum("day", a.SET, ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "away_or_vacation"]),
            ),
        )
...
export const presets = {
    ...
    cover_mode: () =>
        new Composite("cover_mode", "cover_mode", access.ALL)
            ...
            .withFeature(
                new Binary("maintenance", access.ALL, true, false).withDescription(
    ...
    light_brightness_colortemp: (colorTempRange: Range) =>
        new Light().withBrightness().withColorTemp(colorTempRange).withColorTempStartup(colorTempRange),
    light_brightness_colortemp_color: (colorTempRange?: Range, preferHueAndSaturation?: boolean) =>
        new Light()
            .withBrightness()
    ...
    light_brightness_colortemp_colorhs: (colorTempRange: Range) =>
        new Light().withBrightness().withColorTemp(colorTempRange).withColorTempStartup(colorTempRange).withColor(["hs"]),
    light_brightness_colortemp_colorxy: (colorTempRange?: Range) =>
        new Light().withBrightness().withColorTemp(colorTempRange).withColorTempStartup(colorTempRange).withColor(["xy"]),
    ...
    pincode: () =>
        new Composite("pin_code", "pin_code", access.ALL)
            ...
            .withFeature(
                new Enum("user_type", access.SET, ["unrestricted", "year_day_schedule", "week_day_schedule", "master", "non_access"]).withDescription(
